...
class Dve(QMainWindow, Ui_dveWindow):
    ...
    def __init__(self, parent=None):
        ...
        # //////////// Theme selection ///////////
        with open(resource_path("config"+os.sep+"config.json")) as config:
            data = config.read()
    ...
    def setThemeColors(self, THEME):
        path = resource_path("themes"+os.sep+THEME+".json")
    ...
    def launchSearch(self, query):
        ...
        with open(resource_path("temp"+os.sep+"temp.txt"), "w", errors="ignore") as data:
...
class Confgui(QDialog, Ui_FormSettings):
    ...
    def setThemeColors(self, THEME):
        ...
        with open(resource_path("themes"+os.sep+THEME+".json")) as f:
            data = f.read()
        theme = ast.literal_eval(data)
        col1 = theme["col1"]
        col2 = theme["col2"]
    ...
    def onStateChanged(self):
        file_address = resource_path("config"+os.sep+"config.json")
    ...
    def lineColChange(self, text):
        ...
        if self.check_color(text) is True:
            ...
            if source == "lineCol1":
                self.color1Button.setIcon(QIcon(resource_path("img"+os.sep+"stop.png")))
                self.color1Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineCol2":
                self.color2Button.setIcon(QIcon(resource_path("img"+os.sep+"stop.png")))
                self.color2Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineFontCol":
                self.fontColorButton.setIcon(QIcon(resource_path("img"+os.sep+"stop.png")))
    ...
    def listThemeFiles(self):
        path = resource_path("themes"+os.sep)
    ...
    def populateThemeBox(self, theme_files):
        ...
        print(theme_files)
        THEME = self.THEME
        myTheme = resource_path("themes"+os.sep+THEME+".json")
        print("mytheme= "+myTheme)
    ...
    def aceptarAction(self):
        ...
        file_exists = os.path.exists(resource_path("themes"+os.sep+fileName+".json"))
        if file_exists is True:
            file_address = resource_path("themes"+os.sep+fileName+".json")
    ...
    def guardarTema(self, fileName):
        ...
        with open(resource_path("themes"+os.sep+""+fileName+".json", "w")) as new_file:
    ...
    def borrarTemaAction(self):
        ...
        if inUseTheme == fileName:
            ...
            file_exists = os.path.exists("themes"+os.sep+fileName+".json")
            if file_exists is True:
                self.dialogBorrar()
            else:
...
def main():
    ...
    if tray == "True":
        ...
        icon = QIcon("img"+os.sep+"logo-ico.png")
