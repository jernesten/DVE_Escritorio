...
class Dve(QMainWindow):
    ...
    def launchSearch(self, query):
        ...
        with open('temp/temp.txt', 'w') as data:
...
class Confgui(QDialog):
    ...
    def setThemeColors(self, THEME):
        ...
        with open("themes/"+THEME+".json") as f:
            data = f.read()
        theme = ast.literal_eval(data)
        col1 = theme["col1"]
        col2 = theme["col2"]
    ...
    def lineColChange(self, text):
        ...
        if self.check_color(text) is True:
            ...
            if source == "lineCol1":
                self.color1Button.setIcon(QIcon("img/stop.png"))
                self.color1Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineCol2":
                self.color2Button.setIcon(QIcon("img/stop.png"))
                self.color2Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineFontCol":
                self.fontColorButton.setIcon(QIcon("img/stop.png"))
    ...
    def populateThemeBox(self, theme_files):
        THEME = self.THEME
        myTheme = "themes/"+THEME+".json"
        self.themeSelBox.setCurrentIndex(theme_files.index(myTheme))

    def onThemeSelBox_changed(self, value):
        theme_split = value.split("/", 1)
        themeFile = theme_split[1]
        themeFile_split = themeFile.split(".", 1)
        themeName = themeFile_split[0]
    ...
    def aceptarAction(self):
        ...
        file_exists = os.path.exists("themes/"+fileName+".json")
        if file_exists is True:
            file_address = "themes/"+fileName+".json"
    ...
    def guardarTema(self, fileName):        
        ...
        with open("themes/"+fileName+".json", "w") as new_file:
