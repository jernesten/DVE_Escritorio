...
class Dve(QMainWindow, Ui_dveWindow):
    ...
    def __init__(self, parent=None):
        ...
        # //////////// Theme selection ///////////
        with open(HOME+".config/dve/config.json") as config:
            data = config.read()
    ...
    def setThemeColors(self, THEME):
        path = HOME+".config/dve/themes/"+THEME+".json"
    ...
    def launchSearch(self, query):
        ...
        with open(HOME+".config/dve/temp/temp.txt", "w") as data:
...
class Confgui(QDialog, Ui_FormSettings):
    ...
    def setThemeColors(self, THEME):
        with open(HOME+".config/dve/themes/"+THEME+".json") as f:
            data = f.read()
        theme = ast.literal_eval(data)
        col1 = theme["col1"]
        col2 = theme["col2"]
    ...
    def onStateChanged(self):
        file_address = HOME+".config/dve/config.json"
    ...
    def lineColChange(self, text):
        ...
        if self.check_color(text) is True:
            ...
            if source == "lineCol1":
                self.color1Button.setIcon(QIcon(HOME+".local/share/dve/img/stop.png"))
                self.color1Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineCol2":
                self.color2Button.setIcon(QIcon(HOME+".local/share/dve/img/stop.png"))
                self.color2Button.setStyleSheet("background-color:white;")
                self.threeColorsGood()
            elif source == "lineFontCol":
                self.fontColorButton.setIcon(QIcon(HOME+".local/share/dve/img/stop.png"))
    ...
    def aceptarAction(self):
        ...
        file_exists = os.path.exists(HOME+".config/dve/themes/"+fileName+".json")
        if file_exists is True:
            file_address = HOME+".config/dve/themes/"+fileName+".json"
    ...
    def guardarTema(self, fileName):        
        ...
        with open(HOME+".config/dve/themes/"+fileName+".json", "w") as new_file:
    ...
    def dialogBorrar(self):
        ...
        tema_p = HOME+".config/dve/themes/"+tema+".json"
...
def checkExternalFiles():
    files = {
        # "usr/share/img/stop.png" : HOME+".local/share/dve/img/stop.png",
        # "usr/share/img/search.png" : HOME+".local/share/dve/img/search.png",
        # "usr/share/img/logo.png" : HOME+".local/share/dve/img/logo.png",
        # "usr/share/img/logo-ico.png" : HOME+".local/share/dve/img/logo-ico.png",
        # "usr/share/img/i.png" : HOME+".local/share/dve/img/i.png",
        # "usr/share/img/gear.png" : HOME+".local/share/dve/img/gear.png",
        HERE+"usr/share/config/config.json" : HOME+".config/dve/config.json",
        #check temp dir
        HERE+"usr/share/temp/" : HOME+".config/dve/temp/",
        #check themes dir
        HERE+"usr/share/themes/" : HOME+".config/dve/themes/",
    }
...
def createExternalFile(inpath, expath):
    if inpath == HERE+"usr/share/temp/" or inpath == HERE+"usr/share/themes/":
        src = inpath
        dst = expath
...
def main():
    ...
    if tray == "True":
        ...
        icon = QIcon(HERE+"usr/share/img/logo-ico.png")
