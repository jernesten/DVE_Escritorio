...
def checkExternalFiles():
    files = {
        ...
        "config/config.json" : HOME+".config/dve/config.json",
        #check temp dir
        "temp/" : HOME+".config/dve/temp/",
        #check themes dir
        "themes/" : HOME+".config/dve/themes/",
    }
...
def createExternalFile(inpath, expath):
    if inpath == "temp/" or inpath == "themes/":
        src = inpath
        dst = expath
...
def main():
    ...
    if tray == "True":
        ...
        icon = QIcon(HOME+".local/share/dve/img/logo-ico.png")
